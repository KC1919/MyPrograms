...
class Tree
{
    ...
    public void getDiagonals(Node node, List<List<Integer>>list, int level)
    {
        if(node==null)
        return;
        ...
        getDiagonals(node.left,list,level+1);
        getDiagonals(node.right,list,level);
    }
}
